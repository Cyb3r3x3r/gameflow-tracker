...
import { Game, GameStatus, UpdateGameInput } from "@/types/game";

interface EditGameModalProps {
  open: boolean;
  game: Game | null;
  onClose: () => void;
  onSave: (game: UpdateGameInput) => Promise<void>;
}

const statusOptions: GameStatus[] = ["Playing", "Completed", "Backlog"];

export function EditGameModal({ open, game, onClose, onSave }: EditGameModalProps) {
  ...
  const submitGame = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(submitError instanceof Error ? submitError.message : "Failed to update game.");
    } finally {
      setIsSaving(false);
    }
  };
        ...
        >
          ...
          >
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-slate-100">Edit Game</h2>
              <button
                type="button"
                onClick={() => {
                  if (isSaving) return;
...
function Input({
  ...
  return (
    <div>
      <label className="mb-1.5 block text-xs uppercase tracking-wide text-slate-400">{label}</label>
      <input
        value={value}
